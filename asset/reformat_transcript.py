...
def convert_transcript(input_file, output_file):
    ...
    for line in lines:
        ...
        if '→' in line and ':' in line and line.lstrip().replace('→', '').replace(':', '').replace('.', '').isdigit():
            ...
            stripped = line.strip()
            if ':' in stripped and len(stripped) <= 6 and stripped.replace(':', '').isdigit():
                # If we have accumulated text, write it out  
                if current_timestamp and current_text:
                    text = ' '.join(current_text)
                    converted_lines.append(f"{current_timestamp}")
...
if __name__ == "__main__":
    input_file = "asset/transcript_en.txt"
    output_file = "asset/transcript_en_converted.txt"
