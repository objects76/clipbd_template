...
def get_dmenu(
    choices: list[Command], initial_selection: int = 0, timeout_sec: int = 30
) -> str:
    cmd = [
        "rofi",
        "-dmenu",  # '-no-custom',
        "-theme-str",
        "window {width: 10%;} listview {scrollbar: false;} inputbar {enabled: false;}",
        "-lines",
        str(len(choices)),
        "-no-fixed-num-lines",
        "-hover-select",
...
def get_command(dtype: Datatype, data: str | Any) -> Command | None:
    ...
    match dtype:
        case (
            Datatype.YOUTUBE | Datatype.WEBURL | Datatype.HTML_TEXT | Datatype.MARKDOWN
        ):
